# -*- coding: utf-8 -*-
"""TORRESJIMENEZ_Judith_assignment1_part1

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XfuWC8MuHbDe2gqT3rKAF2v5FaQX20kP
...
def question_six(text):
    ...
    for match in matches:
        digits = re.sub(r"\D", "", match)   # keep only numbers
        formatted = f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
        phones.append(formatted)

    return phones # Return list of standardized phone numbers
...
def question_seven():
    ...
    # YOUR CODE HERE
    dates = []
    pattern = r"\d{1,2}/\d{1,2}/\d{2,4}|[A-Za-z]+ \d{1,2}, \d{4}|\d{1,2} [A-Za-z]+ \d{4}"
    ...
    for i, note in enumerate(df):
        ...
        try:
            if match:
                ...
                if year_match:
                    date = pd.to_datetime("1/1/" + year_match.group())
                else:
                    month_year = re.search(r"\d{1,2}/\d{4}", note)
                    if month_year:
                        m, y = month_year.group().split("/")
                        date = pd.to_datetime(f"{m}/1/{y}")
                    else:
                        continue
    ...
    date_df = pd.DataFrame(dates, columns=["index", "date"])
    sorted_df = date_df.sort_values("date")

    return pd.Series(sorted_df["index"].values)
